...
ngOnChanges(changes: { [propKey: string]: SimpleChange }){
    ...
    createColumnDefsEvent(){
        ...
        if(headers.id=="timestamp") {
                ...
                obj["filter"]= 'text';
                obj["width"] = 150;
                obj["filterParams"] = {clearButton: true,filterOptions : ["contains", "equals", "startsWith"]};
            }
            obj["suppressMenu"] = true;
         headersModified.push(obj)  ;
        obj["cellRenderer"] = 'formatValue';
        obj["cellStyle"] = {'text-align':'center'};
         obj["headerClass"] = "text-center";
        }
        }
...
function formatValue(param){
    if(!param || (!param.value && param.value!=0)|| param.value.length==0){
    return '<span>--</span>';
    }else{
