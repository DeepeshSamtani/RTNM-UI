...
export class CommonParsingService {
  ...
  parseRestTableResponse(data) {
    let tableResponse = { headers: [], data: [] };
    tableResponse["headers"] = data.header;
    if(data.header){
      for (let header of tableResponse["headers"]){
    ...
    if (data && data.response && data.response[0] && data.response[0].events && data.response[0].events.length > 0) {
      ...
      // let countergroupList = response.reportConfiguration.counterGroupsWithCounterAndProperties;
      // countergroupList.forEach(countergroup => {
      //   let kpiAndcounterlist : Array<any>=[];
      //   if(countergroup.counterList != undefined && null !=countergroup.counterList )
      //    { kpiAndcounterlist=countergroup.counterList; }
      //   if(countergroup.kpis != undefined && null !=countergroup.kpis) 
      //   {   kpiAndcounterlist= kpiAndcounterlist.concat(countergroup.kpis); }

      //   kpiAndcounterlist.forEach(counter => {
      //     let id = counter.counterKey.counterId + '__' + countergroup.counterGroupId;
      //     let children = [];
      //     let singleAggregation = undefined;
      //     let aggregationList = counter.aggregationList;
      ...
      //       tableResponse["headers"].push({ id: singleAggregation, displayName: counter.displayName + ' (' + counter.counterUnit + ')', value: "number", width: 120 });
      //     } else {
      //       id = counter.aggregationType == 'AVG' ? counter.counterKey.counterId + '_AVG' : counter.counterKey.counterId + '__' + countergroup.counterGroupId;
      //       tableResponse["headers"].push({ id: id, displayName: counter.displayName + ' (' + counter.counterUnit + ')', value: "number", width: 120 });

      //     }
      //   });
      //   countergroup.properties.forEach(property => {
      //     let id = property.propertyKey.propertyId + '__' + countergroup.counterGroupId;
      //     tableResponse["headers"].push({ id: id, displayName: property.propertyName , value: "text", width: 120 });
      //   });
      ...
      // tableResponse["headers"].unshift({ id: "COUNTER_GROUP_ID", displayName: "Counter Group", children: [] });
      ...
      if(data.metrics!=null && data.metrics.length >0){
         for (let metric of data.metrics) {
          ...
          for (let eventOb of response.events) {
              dataInner = [];
              dataInner.push(new Date(eventOb["timestamp"]).getTime(), parseInt(eventOb[metric.counterKey.counterId]));
             console.log("Data inner in getParsedRestChartResponse========"+dataInner);
  ...
  getcounterListForReport(data)
  {
      let reportCounterList : any = new Array();
     if(data.reportConfiguration && data.reportConfiguration.counterGroupsWithCounterAndProperties)
     {
       let counterGroupList = data.reportConfiguration.counterGroupsWithCounterAndProperties;
       if(counterGroupList && null !=counterGroupList && counterGroupList.length >0)
       {
        for(let couterGroup of counterGroupList)
        {
          let counterList = couterGroup.counterList;
          if(counterList && null != counterList && counterList.length >0)
          {
            for(let counter of counterList)
            {
              ...
              reportCounterList.push(obj);
            }
          }
        }
       }
     }
     return reportCounterList;
  }
}