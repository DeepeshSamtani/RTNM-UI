...
export class KpiCreationComponent implements OnInit {
  ...
  formulaString = "";
  kpiDetails = { displayName: "", aggregationType: "", description: "", kpiUnit: "", id: "", formula: "", counterList: "", counterConfig: {},deviceType:"" };
  ...
  ngOnInit() {
    ...
    if (this.route.snapshot.params && this.route.snapshot.params["id"]) {
      let request = {
        counter: {
          counterId: this.route.snapshot.params["id"]
        }
      };
      ...
      let data = {
        ...
        "counter": {
          ...
          "formula": "AMAT7_C_3*AMAT7_C_2"
        },
        "deviceType": "AMAT7",
        "counterId": "AMAT7_C_4",
        "aggregationType": "SUM",
        "counterGroupId": "AMAT7_CG_1",
        "counterList" : ["AMAT7_C_3","AMAT7_C_2"]
      };
      ...
      this.kpiDetails = {
        displayName: data.counter.displayName,
        aggregationType: data.aggregationType,
        description: data.counter.description,
        kpiUnit: data.counter.unit,
        id: data.counterId,
        formula: data.counter.formula,
        counterList: "",
        counterConfig :{},
        deviceType : data.deviceType
      };

      let fm: string = data.counter.formula;
      fm = fm.replace(' ', '');
      fm = fm.replace('+', ',');
      fm = fm.replace('-', ',');
      fm = fm.replace('*', ',');
      ...
      this.kpiDetails.counterList = fm;
      this.kpiDetails["counterConfig"]["counters"] = [{ "displayName": "Counter 1", "counterKey": { "counterId": "AMAT7_C_3" } }, { "displayName": "Counter 1", "counterKey": { "counterId": "AMAT7_C_2" } }];

      this.selectCounterGroupId = data.counterGroupId;
      this.counterIds = data.counterList;
      ...
      this.countersDisplay = this.kpiDetails.counterConfig["counters"];
      // }, (err) => {
      //   console.log(err);
      // });
    } else {
  ...
  editFormulaBuilder() {
    ...
    if (this.kpiDetails.counterConfig != null) {
      counters = this.kpiDetails.counterConfig["counters"];
    }
    ...
    for (let counter of counters) {
      formulaMap.set(counter.counterKey.counterId, "<a class='formulaitem' datavalue='" + counter.counterKey.counterId + "'><$a>");
    }
    ...
    array.each(function (i, element) {
      for (let counter of counters) {
        if (counter.counterKey.counterId == $(element).attr('data-value') && !operator.includes($(element).attr('data-value'))) {
  ...
  loadFormulaBuilder() {
    ...
    $('.formula-advanced').droppable({
      ...
      drop: function (event, ui) {
        ...
        let counterName = count["counterKey"]["counterId"] ? count["counterKey"]["counterId"] : $($element).text().trim();
  ...
  createKPI($event) {
    ...
    var parent = this;
    let formString = "";
    request.counter.unit = this.newKPIForm.get('kpiUnit').value;
    request.aggregationType = this.newKPIForm.get('aggregationType').value;
    request.counter.description = this.newKPIForm.get('kpiDescription').value;
    request.counter.formula = this.formulaString;
    let counterString = "";
    for (let counter of this.countersDisplay) {
      if (parent.formulaString.length > 0 && parent.formulaString.indexOf(counter.counterKey.counterId) >= 0) {
        counterString += counter.counterKey.counterId + ",";
      }
    }
    counterString = counterString.slice(0, -1);
    // request.counterList = counterString;
    request.counter.displayName = this.newKPIForm.get('kpiName').value;
    if (!this.newKPIForm.valid || request.counter.formula.length == 0) {
      return true;
    }
  ...
  getAllKPIs() {
    ...
    // this.kpiList = [{"id":1,"deviceType":"MME","formula":"(1056974608+1056974609)/1056974611","counterList":"1056974608,1056974609,1056974611","displayName":"Kpi Demo","isActive":true,aggregationType:"SUM",kpiUnit:"times",kpiDescription:""}];
    this.kpiList = [{
      "logicalName": "one more KPI",
      "counter": {
        "displayName": "one more KPI",
    ...
    this.kpiList.forEach((e) => {
      ...
      obj["displayName"] = e.counter.displayName;
      obj["deviceType"] = e.deviceType;
      obj["formula"] = e.counter.formula;
      obj["isActive"] = e.counter.isActive == 1 ? true : false;
      obj["id"] = e.counterId;
      obj["link"] = e.counterId;
      console.log(e.formula);
      this.eventData.data.push(obj);
    });
  ...
  removeCounter() {
    ...
    //   return c.counterKey.counterId != counter.counterKey.counterId
    // })
    ...
    for (let cg of this.deviceTypeModel.controlgroup) {
      if (cg.counterGroupId == this.selectCounterGroupId) {
        for (let count of cg.counterList) {
          if (this.countersDisplay.indexOf(count.counterKey.counterId) >= 0) {
            count["showEnable"] = false;
            break cgloop;
          }
        }
      }
  ...
  gotoKPIGenerationFixed(id, menu) {
    if (id == 'kpiProp' && menu == 'next') {
      this.kpiTabMenu = 'kpiCalc';
      $('#' + 'kpiCalculation').click();
    } else if (id == 'kpiCalc' && menu == 'next') {
      $('#' + 'kpiListAnchor').click();
      this.getAllKPIs();
      this.kpiTabMenu = 'kpiList';
    } else if (id == 'kpiCalc' && menu == 'previous') {
      $('#' + 'kpiConfigTab').click();
      this.kpiTabMenu = 'kpiProp';
    } else if (id == 'kpiList' && menu == 'previous') {
      $('#' + 'kpiCalculation').click();
      this.kpiTabMenu = 'kpiCalc';
    }
  }
